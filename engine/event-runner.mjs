...
// ---------- 계절 (clock.json 정본 — 규칙 25) ----------
function seasonNow() {
  return clock.seasons?.[String(now().getMonth() + 1)] ?? '여름';
}

// ---------- 예정 사건 (규칙 24 — WO-014) ----------
const readUpcoming = () => readJson('world/upcoming.json');
function dueUpcoming() {
  return readUpcoming().filter((u) => u.status === 'pending' && new Date(u.due) <= now());
}
...
## 현재 world/ (진실)
${['characters.json', 'places.json', 'factions.json', 'axes.json', 'timeline.json', 'upcoming.json']
    .map((f) => `### world/${f}\n${read(`world/${f}`)}`).join('\n')}
...
- 계절: ${seasonNow()} (규칙 25 — weather·서술 모두 이 계절과 정합할 것)
...
${due.length ? `- **도래한 예정 사건 — 이번 이벤트는 반드시 이것의 결과를 다뤄라 (규칙 24-1)**: ${due.map((u) => `${u.id} "${u.title}" (기한 ${u.due})`).join(' / ')}. 결과는 창발이다 — 성사·변형·연기·무산 모두 가능하며, 무산도 사건이다. upcoming_resolve에 해당 id를 반드시 넣어라.` : ''}
...
(JSON 객체 1개, 코드펜스 없이. 필드:
  ...
  "weather": "맑음|흐림|비|바람|안개|눈 중 하나 — 계절 정합, 직전 이벤트에서 자연스럽게 (규칙 22-2)",
  "truth": "...", "public_knowledge": "...",
  "dialogue": [ { "speaker": "인물 id 또는 무명 호칭", "line": "..." } ]  (대화 없으면 []),
  "staging": [ { "actor": "인물 id 또는 무명 호칭", "move_to": "표준 장소 슬러그 11곳만 — temple|oldtown|smithy|apple-yard|bakery|market|port-lane|fisher-hut|dock|saltfields|mudflat-huts (규칙 22-1)", "action": "동사(예: talk, work, walk, sit)" } ],
  "affected": [ 인물/구역/세력 id ],
  "state_diff_summary": "상태 변화 요약 한 줄 (없으면 '없음')",
  "timeline_id": "등급 1+일 때만 — timeline용 영문 슬러그",
  "upcoming_new": [ { "title": "...", "due": "ISO8601 미래 시각", "note": "누가 왜 기약했나" } ]  (이번 사건이 자연스러운 기약을 낳았을 때만 — 규칙 24, 주 0~2건 권장. 없으면 []),
  "upcoming_resolve": [ { "id": "up-...", "outcome": "성사|변형|연기|무산" } ]  (이번 사건이 예정의 결과일 때만. 없으면 []) )
...
function validateAndApply(text) {
  ...
  // weather (규칙 22-2) — 표준 어휘 + 계절 정합(6~8월 눈 금지)
  const WEATHER = ['맑음', '흐림', '비', '바람', '안개', '눈'];
  if (!WEATHER.includes(ev.weather)) throw new Error(`weather '${ev.weather}' 무효 — ${WEATHER.join('|')} 중 하나`);
  if (ev.weather === '눈' && ['봄', '여름'].includes(seasonNow()))
    throw new Error(`계절 위반: ${seasonNow()}의 눈 (규칙 22-2·25, clock.json 계절 정본)`);
  // 예정 사건 (규칙 24 — WO-014)
  const upcoming = readUpcoming();
  const due = dueUpcoming();
  const resolves = ev.upcoming_resolve ?? [];
  for (const r of resolves) {
    const u = upcoming.find((x) => x.id === r.id);
    if (!u) throw new Error(`upcoming_resolve id '${r.id}' 미등록`);
    if (u.status !== 'pending') throw new Error(`upcoming '${r.id}'는 이미 ${u.status}`);
    if (!['성사', '변형', '연기', '무산'].includes(r.outcome)) throw new Error(`outcome '${r.outcome}' 무효`);
  }
  if (due.length && !resolves.some((r) => due.some((u) => u.id === r.id)))
    throw new Error(`도래한 예정(${due.map((u) => u.id).join(', ')})을 다루지 않았다 — 규칙 24-1 위반`);
  const news = ev.upcoming_new ?? [];
  for (const n of news) {
    if (!n.title || !n.due || isNaN(new Date(n.due))) throw new Error('upcoming_new에는 title과 유효한 due(ISO8601) 필수');
    if (new Date(n.due) <= now()) throw new Error(`upcoming_new due '${n.due}'가 미래가 아니다`);
  }
  const thisWeekCount = upcoming.filter((u) => u.week === v.week).length;
  if (news.length && thisWeekCount + news.length > 2)
    throw new Error(`예정 등록 초과: 이번 주 이미 ${thisWeekCount}건 (주 최대 2건 — 규칙 24)`);
  // staging 장소 표준 어휘 (규칙 22-1)
  const PLACES = ['temple', 'oldtown', 'smithy', 'apple-yard', 'bakery', 'market', 'port-lane', 'fisher-hut', 'dock', 'saltfields', 'mudflat-huts'];
  for (const st of ev.staging ?? [])
    if (!PLACES.includes(st.move_to)) throw new Error(`staging 장소 '${st.move_to}' 비표준 — 규칙 22-1의 11곳만`);

  const REVERTIBLE = [...STATE_FILES, 'upcoming.json'];
  const before = Object.fromEntries(REVERTIBLE.map((f) => [f, read(`world/${f}`)]));
  const revert = () => { for (const f of REVERTIBLE) write(`world/${f}`, before[f]); };
  try {
    ...
    const entry = {
      ...
      ts: now().toISOString(), week: v.week, grade: ev.grade, weather: ev.weather,
    ...
    // 예정 사건 갱신 (규칙 24-2) — 해소 먼저, 신규 등록 다음
    if (resolves.length || news.length) {
      const up = readUpcoming();
      for (const r of resolves) {
        const u = up.find((x) => x.id === r.id);
        u.status = 'resolved'; u.resolved_by = entry.id; u.outcome = r.outcome; u.resolved_ts = entry.ts;
      }
      for (const n of news) {
        up.push({
          id: `up-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
          title: n.title, due: new Date(n.due).toISOString(), origin_event: entry.id,
          note: n.note ?? '', status: 'pending', resolved_by: null, week: v.week, created_ts: entry.ts,
        });
      }
      write('world/upcoming.json', JSON.stringify(up, null, 2) + '\n');
    }
